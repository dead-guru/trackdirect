...
L.Maidenhead = L.LayerGroup.extend({
    ...
    onAdd: function (map) {
        ...
        this._map.on(this.options.redraw, function () {
            grid.redraw(this._map);
        });
    ...
    onRemove: function (map) {
        this._map.off(this.options.redraw, this.redraw, this);
    ...
    _getLabel: function(lon,lat) {
        ...
        var title = '<span style="cursor: default; color:'+this.options.color+'; font-size:'+size+'; font-weight: 900;">' + this._getLocator(lon,lat) + '</span>';
        var myIcon = L.divIcon({className: 'locator', html: title});
