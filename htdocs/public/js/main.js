  ...
  overviewStart: function () {
    ...
    this._activeCallback = function(packet) {
      ...
      $("#packet_type_name, #latest-timestamp, #latest-timestamp-age, #raw_path, #latest-packet-comment").fadeOut(250, function(){
        $("#packet_type_name").text('Пакет ' + packet.getPacketTypeName());
      ...
      if (packet.packet_type_id == 1) {
        ...
        if (packet.speed != null) $("#latest_speed").fadeOut(250, function(){ $("#latest_speed").html(trackdirect.isImperialUnits() ?  Math.round(trackdirect.services.imperialConverter.convertKilometerToMile(packet.speed)*100)/100 + " mph" : Math.round(packet.speed*100)/100 + " км/г").fadeIn(250);});
